...
const MyTask = () => {
  ...
  const getMandate = async () => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/Workflow/GetMyTasks?pageNumber=1&pageSize=10&sort={}&filter=[]`,
        config
      )
      .then((response: any) => {
...
const selfSubmit =() => {
    ...
    userActionList?.find(
      (item) =>
        item?.mandateId === columnData?.id &&
        item?.module === columnData?.url
    );
...
const assignedToPM = (e) => {
  ...
  const handleExportData = () => {
    axios
    .get(
      `${process.env.REACT_APP_BASEURL}/api/Phases/GetExcelDownloadMyTasks`
    )
      .then((response) => {
        if (!response) {
        ...
        if (response?.data) {
          ...
          if (typeof window.navigator.msSaveBlob !== "undefined") {
            ...
            dispatch(
              showMessage("MyTask downloaded successfully!")
            );
          }
        }
      });
    ...
    <>
      ...
      >
        ...
        <DialogActions className="button-wrap">
                ...
                <Button className="yes-btn" onClick={(e) => {radioButton === "Self" ? selfSubmit() : assignedToPM(e)}} >
                  Submit
                </Button>
