...
const ScopeOfWork = ({
  ...
  useEffect(() => {
    if (phaseData) {
      setPhaseApprovalNoteScopeData(phaseData);
    }
  }, [phaseData]);
  ...
  const getScope = async () => {
    await axios
      .get(
        ...
        }/api/PhaseApprovalNote/PhaseApprovalNoteScope?noteid=${
          phaseApprovalNoteId || 0
        }&notetype=Phase`
      )
      .then((response: any) => {
        if (!response) return;
      ...
      cellRenderer: (e1: any) => (
        <>
          <div style={{ width: "200px" }}>
            <ToggleSwitch
              alignment={
                phaseData?.[e1?.rowIndex]?.include === "0" ? false : true
              }
              handleChange={(e) => {
                var data = [...e1?.phaseData];
                data[e1?.rowIndex].include =
              ...
              id="non_budgeted"
              onBlur={() => {}}
              disabled={actionName === "Approve" || pathName === "phase-approval-note-view"}
            />
          </div>
        </>
      ),
