...
const MyTask = () => {
  ...
  const getMandate = async () => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/Mandates/BulkMandatePDMAssign?PhaseId=${mandateData?.id}&PDMID=${user?.id}`,
        config
      )
      .then((response: any) => {
  ...
  useEffect(() => {
    var reporting_id = user?.id || 0;
    if (mandateData?.verticalname !== "") {
      axios
        .get(
          `${process.env.REACT_APP_BASEURL
          }/api/User/GetAllWithConditions?Designation=Project Manager&reporting_id=${reporting_id}`
        )
        .then((response: any) => {
          if (!response) return;
  ...
  }, [mandateData?.verticalname, mandateData?.fk_PDM_id]);
  ...
  let columnDefs = [
    ...
    const handleSubmit = () => {
        if (pmId === null) {
            setPmError('Please select Project Manager');
            return;
        }
        ...
        const data =
            ...
            payload?.length &&
            payload?.map((item) => {
                item['projectManagerId'] = pmId?.id || 0;
                return item;
            });
        ...
        axios
            .post(`${process.env.REACT_APP_BASEURL}/api/Mandates/BulkMandatePMAssign`, data)
            .then((response: any) => {
                setChecked({});
                getMandate();
                handleClose();
                dispatch(showMessage('Bulk Assign Project manager is completed successfully!'));
                navigate("/Mandate")	
    ...
    <>
      ...
      >
        <MandateInfo
          ...
          workItem={"Assign PM To Mandate"}
        />
        {((assignedPmData && Object?.values(assignedPmData)?.length !== 0) ||
          selectAllBox) && (
            <div>
              <Button
