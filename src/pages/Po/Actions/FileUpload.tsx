...
const FileUploadAction = ({
  ...
  const handleUploadFiles = async (e, files) => {
    ...
    form.append("entityname", "PO");
    form.append("RecordId", selectedItem?.id || 0);
      ...
      axios
        ...
        .then((response: any) => {
          ...
          if (response?.data?.data == null) {
            ...
            _selectedItem.is_file_upload = true;
            updateFileUploadFlag(_selectedItem);
            setUploadedFiles([]);
            setFileLimit(false);
  ...
  return (
    <>
      {" "}
      <Tooltip
        id={`${selectedItem?.id}_tooltipid`}
        className="actionsIcons"
        title={`Upload Document ${
          selectedItem.is_file_upload ? "" : "(Not Available)"
        }`}
      >
