...
const FileDownloadList = ({ props, mandate, setTableData, setFormData }) => {
    ...
    const getVersionHistoryData = () => {
        ...
        axios
            .get(
                `${process.env.REACT_APP_BASEURL}/api/ImageStorage/GetDocUploadHistory?mandateid=${mandate?.id}&documentType=PO`
            )
            .then((response: any) => {
                if (!response) {
                ...
                if (response?.data && response?.data && response?.data?.length > 0) {
                    ...
                    data = data && data?.length > 0 && data?.filter(item => item?.recordId === props?.id && parseInt(props?.id));
