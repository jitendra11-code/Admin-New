...
const MandateBulkUpload = () => {
    ...
    useEffect(() => {
        if (documentTypeList && documentTypeList?.length > 0) {
            var obj = documentTypeList && documentTypeList.find((item) => item?.documentName === 'Mandate List');
            setDocType(obj || null);
        }
    }, [documentTypeList]);
            ...
            axios
                ...
                .then((response: any) => {
                    ...
                    if (response?.data?.data == null) {
                        ...
                        getVersionHistoryData();
                        axios
                            .post(`${process.env.REACT_APP_BASEURL}/api/Mandates/UploadAndSaveExcelData`, formDataTemp)
                            .then((res) => {
                                if (!res) {
                                    e.target.value = null;
    ...
    const downloadFileLogFile = (data) => {
        axios
            .get(`${process.env.REACT_APP_BASEURL}/api/Mandates/DownloadLogData?uploadNo=${data?.recordId || 0}&Type=${value === 1 ? 'Completed' : 'Error'}`)
            .then((response: any) => {
                if (!response) {
                    dispatch(fetchError('Error Occurred !'));
                    return;
                ...
                if (response?.data) {
                    ...
                    if (typeof window.navigator.msSaveBlob !== 'undefined') {
                        ...
                        link.click();
                        dispatch(showMessage('The Mandate Bulk Upload Log file is downloaded successfully!'));
                    }
                }
            })
            .catch((e: any) => {
...
export default MandateBulkUpload;
