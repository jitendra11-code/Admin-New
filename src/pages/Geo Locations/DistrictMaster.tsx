...
import { regExpressionStringField, textFieldValidationOnPaste } from '@uikit/common/RegExpValidation/regForTextField';
...
export default function DistrictMaster() {
    ...
    useEffect(() => {
        if (editFlag) {
            ...
            setFieldValue('districtName', EditMaster?.districtName);
        }
    ...
    const validateSchema = Yup.object({
        districtName: Yup.string().required('Please enter District Name'),
    ...
    const { values, handleBlur, setFieldValue, handleChange, handleSubmit, errors, touched, resetForm } = useFormik({
        ...
        onSubmit: (values, action) => {
            ...
            if (editFlag) {
                axios
                    .post(`${process.env.REACT_APP_BASEURL}/api/DistrictMaster/InsertUpdateDistrictMaster`, body)
                    .then((response: any) => {
                        if (!response) {
                            dispatch(fetchError('District Name not updated'));
                            setOpen(false);
                            return;
                        }
                        if (response && response?.data?.code === 200 && response?.data?.status === true) {
                            dispatch(showMessage('District Name updated successfully'));
                            setOpen(false);
                            showRightData();
                            return;
                        } else {
                            dispatch(fetchError('District Name already exists'));
                            setOpen(false);
                            return;
                        }
                    })
                ...
                axios
                    ...
                    .then((response: any) => {
                        if (!response) {
                            dispatch(fetchError('Please fill all mendatory fields'));
                            // setOpen(false);
                            return;
                        }
                        if (response && response?.data?.code === 200 && response?.data?.status === true) {
                            dispatch(showMessage('District Name added successfully'));
                            setOpen(false);
                            showRightData();
                            // getMyGenerateId();
                            return;
    ...
    const showRightData = async () => {
        ...
        await axios
            .get(`${process.env.REACT_APP_BASEURL}/api/DistrictMaster/GetDistrictMasterList`)
            .then((response: any) => {
    ...
    const getTitle = () => {
        if (editFlag) {
            return 'Update District Name Details ';
        } else {
            return 'Add District Name Details';
        }
    };

    const handleExportData = () => {
        axios.get(`${process.env.REACT_APP_BASEURL}/api/DistrictMaster/GetExcelReport`).then((response) => {
            if (!response) {
                dispatch(fetchError('Error occurred in Export !!!'));
                return;
            }
            if (response?.data) {
                var filename = 'DistrictMaster.xlsx';
                ...
                if (typeof window.navigator.msSaveBlob !== 'undefined') {
                    ...
                    dispatch(showMessage('District Excel downloaded successfully!'));
                }
            }
        });
    };
                                    ...
                                    <DialogContent style={{ width: '450px' }}>
                                        ...
                                        >
                                            ...
                                            {stateDropdown?.map((v: any) => (
                                                <MenuItem value={v?.id}>{v?.stateName}</MenuItem>
                                            ))}
                                        </Select>
                    ...
                    /> */}
                      ...
                      <p className="form-error">{errors.state}</p>
                                        ...
                                        <TextField
                                            ...
                                            onChange={handleChange}
                                        />
                                        {touched.districtName && errors.districtName ? <p className="form-error">{errors.districtName}</p> : null}
                                    </DialogContent>
