...
const FileDownloadList = ({ props, recordId, mandate }) => {
    ...
    const getVersionHistoryData = (recordId) => {
        ...
        axios
            .get(
                `${process.env.REACT_APP_BASEURL}/api/ImageStorage/GetDocUploadHistory?mandateid=${mandate?.id}&documentType=Quality Audit Report&recordId=${recordId || 0}`
            )
            .then((response: any) => {
                if (!response) {
    ...
    React.useEffect(() => {
        ...
        if (mandate?.id !== undefined) {
            ...
            getVersionHistoryData(recordId);
        }
    ...
    return (
        <> <Tooltip className='actionsIcons'
            id={`${recordId}_viewordownload_photos`}
