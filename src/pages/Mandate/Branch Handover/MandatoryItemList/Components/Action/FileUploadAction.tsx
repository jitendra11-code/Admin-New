...
const MAX_COUNT = 8;
const FileUploadAction = ({ params, getDocData }) => {
    ...
    const handleUploadFiles = async (e, files, params) => {
        ...
        formData.append("documenttype", "Mandatory Item List and Pictures");
        formData.append("CreatedBy", user?.UserName || "");
        formData.append("ModifiedBy", user?.UserName || "");
        formData.append("entityname", "Mandatory Item List and Pictures");
        formData.append("RecordId", params?.data?.SEQUENCE || 0);
    ...
    return (<> <Tooltip
        ...
        title={(params?.data?.Availability === "Yes" && params?.data?.isDocumentUploaded === false) ? "Upload Photos (Not Available)" : "Upload Photos"}>
        <AddPhotoAlternateOutlinedIcon
            style={params?.data?.Availability === "" || params?.data?.Availability === "No" ? { pointerEvents: "none", opacity: 0.5, fontSize: '20px', color: '#000' } : { fontSize: '20px', color: '#000', cursor: 'pointer', }}
            onClick={() => {
                if (params?.mandate?.id === undefined) {
