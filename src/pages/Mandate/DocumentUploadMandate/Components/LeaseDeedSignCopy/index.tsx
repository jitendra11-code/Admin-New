  ...
  return (
    ...
    >
      ...
      <TabContext value={value}>
        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
          <TabList onChange={handleChange} aria-label="lab API tabs example">
            <Tab
              value="1"
              label="Generate LOA"
              to={`/mandate/${id}/generate-loa?source=list`}
              component={Link}
            />
            <Tab
              value="2"
              label="Generate LOI"
              to={`/mandate/${id}/generate-loi?source=list`}
              component={Link}
            />
            <Tab
              value="3"
              to={`/mandate/${id}/generate-tsr?source=list`}
              label="Generate TSR"
              component={Link}
            />
            <Tab
              ...
              to={`/mandate/${id}/additional-documents?source=list`}
              component={Link}
            />
            <Tab
              value="5"
              label="RBI Intimation"
              to={`/mandate/${id}/rbi-intimation?source=list`}
              component={Link}
            />
            <Tab
              value="6"
        ...
        <UploadDocuments documentType={"Lease Deed Sign Copy"} />
      </TabContext>
    </div>
  );
};
