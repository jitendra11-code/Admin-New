...
const notesValue = {
  key: "Certificates and Additional Documents",
  name: "Certificates and Additional Documents",
};
const MAX_COUNT = 8;

const CertificatesAndAdditionalDocuments = () => {
  ...
  const workFlowMandate = () => {
    ...
    axios({
      ...
      .then((response: any) => {
        dispatch({
          type: SHOW_MESSAGE,
          message: "Certificates and Additional Documents added Successfully!",
        });
        if (!response) return;
  ...
  const getVersionHistoryData = () => {
    axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/ImageStorage/GetDocUploadHistory?mandateid=${mandateId?.id}&documentType=Certificates and Additional Documents`
      )
      .then((response: any) => {
        if (!response) {
          dispatch(fetchError("Error Occurred !"));
  ...
  const handleUploadFiles = async (e, files) => {
    ...
    formData.append("documenttype", "Certificates and Additional Documents");
    formData.append("CreatedBy", user?.UserName || "");
    formData.append("ModifiedBy", user?.UserName || "");
    formData.append("entityname", "Certificates and Additional Documents");
...
export default CertificatesAndAdditionalDocuments;
