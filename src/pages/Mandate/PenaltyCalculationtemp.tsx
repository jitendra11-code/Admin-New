...
const NDC = ({ mandateId, currentStatus, currentRemark }) => {
  ...
  const downloadTemplate = (docType) => {
    axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/ImageStorage/DownloadTemplate?documenttype=No Due Certificate`,
        {
  ...
  const handleUploadFiles = async (e, files) => {
    ...
    formData.append("documenttype", "No Due Certificate");
    formData.append("CreatedBy", user?.UserName || "");
    formData.append("ModifiedBy", user?.UserName || "");
    formData.append("entityname", "No Due Certificate");
  ...
  const getVersionHistoryData = () => {
    axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/ImageStorage/GetDocUploadHistory?mandateid=${mandateId?.id}&documentType=No Due Certificate`
      )
      .then((response: any) => {
        if (!response) {
          dispatch(fetchError("Error Occurred !"));
