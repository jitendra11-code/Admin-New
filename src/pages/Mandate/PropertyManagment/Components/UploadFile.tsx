...
import { Typography, Grid, Button, TextField, Autocomplete, Stack } from '@mui/material';
...
const MAX_COUNT = 8;
const UploadDocuments = ({ documentType, setMID }) => {
    ...
    const getActivityCompletedStatus = () => {
        if (projectPlanAPIData && projectPlanAPIData?.length > 0 && projectPlanData && projectPlanData?.length > 0) {
            ...
            } else if (status_completed?.length === projectPlanAPIData?.length) {
                console.log('ActivityFlag', status_completed?.length, projectPlanAPIData?.length);
    ...
    }, [projectPlanAPIData, projectPlanData]);
    ...
    const _bulkdownloadaZip = () => {
        ...
        axios
            .post(`${process.env.REACT_APP_BASEURL}/api/ImageStorage/DownloadZipFile?mandateid=${mandateId?.id}`, list, {
    ...
    const workFlowMandate = (action = '') => {
        ...
        const body = {
            ...
            action: action === '' ? 'Created' : action === 'Activity Completed' ? action : 'Created',
        };
        axios({
            method: 'post',
    ...
    const getVersionHistoryData = () => {
        ...
        axios
            .get(`${process.env.REACT_APP_BASEURL}/api/ImageStorage/GetDocUploadHistory?mandateid=${mandateId?.id}&documentType=${documentType}`)
            .then((response: any) => {
                if (!response) {
                    dispatch(fetchError('Error Occurred !'));
                    return;
    ...
    useEffect(() => {
        if (mandateId?.id !== undefined && docType?.documentType !== undefined) {
            if (documentType && documentType?.trim() !== 'Project Plan') getVersionHistoryData();
        }
    ...
    const _saveProjectPlanHistory = (_data) => {
        const data =
            ...
            _data.map((item) => {
                return {
                    ...
                    mandateId: mandateId?.id || 0,
                    proposed_Start_Date: (moment(item?.proposed_Start_Date).isValid() && moment(item?.proposed_Start_Date).format('YYYY-MM-DDTHH:mm:ss.SSS')) || null,
                    proposed_End_Date: (moment(item?.proposed_End_Date).isValid() && moment(item?.proposed_End_Date).format('YYYY-MM-DDTHH:mm:ss.SSS')) || null,
                    actual_Start_Date: (moment(item?.actual_Start_Date).isValid() && moment(item?.actual_Start_Date).format('YYYY-MM-DDTHH:mm:ss.SSS')) || null,
                    actual_End_Date: (moment(item?.actual_End_Date).isValid() && moment(item?.actual_End_Date).format('YYYY-MM-DDTHH:mm:ss.SSS')) || null,
                    createdBy: user?.UserName,
        ...
        axios
            .post(`${process.env.REACT_APP_BASEURL}/api/ProjectPlan/CreateProjectPlanHistory`, data, {})
            .then((response: any) => {
                if (!response) return;
    ...
    const _saveOrUpdateProjectPlanData = (action = '') => {
        var _validationArr;
        if (user && user?.role && user?.role?.trim() === 'Project Manager') {
            _validationArr = projectPlanData && projectPlanData?.filter((item) => item?.proposed_Start_Date === null || item?.proposed_End_Date === null || item?.process_Owner === '');
        } else {
            var _validationArr2 =
                projectPlanData &&
                projectPlanData?.filter((item) => {
                    if (item?.status_percentage > 0) {
                        return item?.actual_Start_Date === null;
                    }
                });
            var _validationArr3 =
                projectPlanData &&
                projectPlanData?.filter((item) => {
                    if (item?.status_percentage >= 100) {
                        return item?.actual_End_Date === null;
                    }
                });
            var _validationArr4 = projectPlanData && projectPlanData?.filter((item) => item?.actual_Start_Date === null || item?.actual_End_Date === null);
        }
        ...
        if (_validationArr2 && _validationArr2?.length > 0) {
            dispatch(fetchError('Please select Actual Start Date !!!'));
            return;
        }
        if (_validationArr3 && _validationArr3?.length > 0) {
            dispatch(fetchError('Please select Actual End Date !!!'));
            return;
        }
        if (_validationArr4 && _validationArr4?.length === projectPlanData?.length) {
            dispatch(fetchError('Please select at least one line Actual Start Date and Actual End Date !!!'));
            return;
        }

        if (projectPlanDataUpdated === false) {
            const projectPlanData1: any = projectPlanData?.map((v) => delete v?.listName);
            ...
            axios
                .post(`${process.env.REACT_APP_BASEURL}/api/ProjectPlan/CreateProjectPlan`, data, {})
                .then((response: any) => {
                    if (!response) return;
                    if (response && response?.status === 200) {
                        dispatch(showMessage('Project Plan is created successfully!'));
                        _getProjectPlanDataForHistory();
                        workFlowMandate(action);
                    } else {
                        dispatch(fetchError('Error occured'));
                        return;
                    }
            ...
            axios
                .post(`${process.env.REACT_APP_BASEURL}/api/ProjectPlan/UpdateProjectPlan`, data, {})
                .then((response: any) => {
                    if (!response) return;
                    if (response && response?.status === 200) {
    ...
    const getMileStone = () => {
        ...
        axios
            .get(`${process.env.REACT_APP_BASEURL}/api/ITAsset/GetITAssets?Type=Project Plan&PartnerCategory=${user?.vendorType || ''}`)
            .then((res) => {
                if (!res) return;
    ...
    const _getProjectPlanDataForHistory = () => {
        axios
            .get(`${process.env.REACT_APP_BASEURL}/api/ProjectPlan/GetProjectPlan?mandateId=${mandateId?.id || 0}&vendorType=${user?.vendorType || ''}`)
            .then((res) => {
                if (!res) return;
                if (res && res?.data && res?.data?.length > 0) {
                    var _data =
    ...
    useLayoutEffect(() => {
        if (mandateId && mandateId?.id !== undefined && mandateId?.id !== 'noid' && documentType && documentType?.trim() === 'Project Plan') {
            getProjectPlan();
        }
    }, [mandateId, docType]);
    console.log('activityFlg', activityFlg, action, runtimeId);
    return (
        <div className={window.location.pathname?.includes('project-management') ? 'inside' : 'inside-scroll-298'} style={{ padding: '5px' }}>
            <MandateInfo mandateCode={mandateId} source="" pageType="" setMandateData={setMandateData} setMandateCode={setMandateId} setCurrentStatus={setCurrentStatus} setCurrentRemark={setCurrentRemark} redirectSource={`${params?.source}`} setpincode={() => {}} />
            <div>
...
text/plain, application/pdf, image/*"
                ...
                {mandateId?.id !== undefined && documentType && documentType?.trim() === 'Project Plan' && (
                    <div style={{ marginBottom: 10 }}>
                        <ProjectPlan
                            mandate={mandateId}
                            getActivityCompletedStatus={getActivityCompletedStatus}
                ...
                {mandateId && mandateId?.id && mandateId?.id !== undefined && documentType && documentType?.trim() !== 'Project Plan' && (
            ...
            <div className="bottom-fix-btn" style={{ marginBottom: 22 }}>
                <div className="remark-field">
                    <Stack display="flex" flexDirection="row" justifyContent="space-between" alignItems={'center'} alignContent="center">
                            ...
                            <>
                                <Button
                                    ...
                                    onClick={(e) => {
                                        ...
                                        if (docUploadHistory && docUploadHistory?.length === 0 && documentType && documentType?.trim() !== 'Project Plan') {
                                            dispatch(fetchError('Please upload file'));
                                            return;
                                        }
                                        if (projectPlanData && projectPlanData?.length === 0 && documentType && documentType?.trim() === 'Project Plan') {
                                            dispatch(fetchError('Please add project plan'));
                                            return;
                                        }
                        ...
                        {activityFlg && (action === 'Create' || action === 'Upload') && runtimeId !== 0 && (
                            <Button
                                variant="outlined"
                                size="medium"
                                name="submit"
                                disabled={!activityFlg}
                                onClick={(e) => {
                                    _saveOrUpdateProjectPlanData('Activity Completed');
                                }}
                                style={{
                                    marginLeft: 10,
                                    padding: '2px 20px',
                                    borderRadius: 6,
                                    color: '#fff',
                                    borderColor: !activityFlg ? '' : '#00316a',
                                    backgroundColor: !activityFlg ? 'gray' : '#00316a',
                                }}
                            >
                                Activity Completed
                            </Button>
            ...
            <div className="bottom-fix-history" style={{ bottom: '75px' }}>
                <MandateStatusHistory mandateCode={mandateId?.id} accept_Reject_Remark={currentRemark} accept_Reject_Status={currentStatus} />
            </div>
                        ...
                        {action && action?.trim() === 'Approve or Reject' && (
                            <>
                                <ApproveSendBackRejectAction
                                    ...
                                    rejectEvent={() => {
                                        workflowFunctionAPICall(runtimeId, mandateId?.id, remark, 'Rejected', navigate, user);
                                    }}
                                />
                                <span className="message-right-bottom">{userAction?.stdmsg}</span>
                            </>
