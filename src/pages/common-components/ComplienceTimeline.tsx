...
const Timeline = ({ mandateCode }) => {
  ...
  const getData = (mandateCode) => {
    axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/ComplianceIntimationRequest/GetTimeLinesForCompliance?id=${mandateCode}`
      )
      .then((response) => {
  ...
  const handleExportData = (id) => {
    axios
      .get(
        `${process.env.REACT_APP_BASEURL}//api/ComplianceIntimationRequest/GetExcelForTimeLines?id=${id}`
      )
      .then((response) => {
        if (!response) {
        ...
        if (response?.data) {
          ...
          if (typeof window.navigator.msSaveBlob !== "undefined") {
            ...
            dispatch(
              showMessage("Timeline Excel downloaded successfully!")
            );
          }
        }
      });
  ...
  const sendMail = () => {
    axios
      .post(
        ...
        }/api/Workflow/SendPendingEmail?HistoryId=${sendMailId || 0}`,
        {}
      )
      .then((response) => {
        if (!response) {
