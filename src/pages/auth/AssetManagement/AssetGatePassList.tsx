...
function AssetGatePassList() {
  ...
  const handleExportDataViewRequest = () => {
    axios
      .get(
      ...
      .then((response) => {
        ...
        if (response?.data) {
          ...
          if (typeof window.navigator.msSaveBlob !== "undefined") {
            ...
            dispatch(showMessage("ViewRequest downloaded successfully!"));
          }
        }
      });
  };
  ...
  const getAssetList = async () => {
    await axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/AssetGatePass/GetAssetGatePassDetails`
      )
      .then((response: any) => {
      ...
      cellRenderer: (e: any) => (
        <>
          <div className="actions">
            <Tooltip title={e?.data?.courier_Person_Name.length > 0 ? "" : "Edit Gate Pass"} className="actionsIcons">
              <button
                className="actionsIcons actionsIconsSize"
                hidden={_validationIdentifierList.includes("request_edit_icon")}
                disabled={e?.data?.courier_Person_Name.length > 0}
                style={{ backgroundColor: e?.data?.courier_Person_Name.length > 0 ? 'gray' : '' }}
              >
                <TbPencil
                  onClick={() =>
                    // console.log("eval",e)
                    navigate(
                      `/${e?.data?.id}/AssetGatePass`
            ...
            >
              <button className="actionsIcons actionsIconsSiz">
                <VisibilityIcon
                  ...
                  onClick={() =>
                    navigate(
                      `/${e?.data?.id}/AssetGatePass?action=view`
                    )
                  }
                />
              </button>
  ...
  const handleExportData = () => {
    axios
    .get(
      `${process.env.REACT_APP_BASEURL}/api/AssetGatePass/GetExcelForAssetGatePass`
    )
      .then((response) => {
        if (!response) {
          dispatch(fetchError("Error occurred in Export !!!"));
        ...
        if (response?.data) {
          var filename = "AssetGatePassList.xlsx";
          if (filename && filename === "") {
            dispatch(fetchError("Error Occurred !"));
            return;
          }
          ...
          if (typeof window.navigator.msSaveBlob !== "undefined") {
            ...
            dispatch(
              showMessage("Asset Gate Pass List downloaded successfully!")
            );
          }
        }
      });
          ...
          >
            ...
            <Tooltip title="Add New">
              <Button
                ...
                onClick={() => navigate("/noid/AssetGatePass")}
              >
                Add New
              </Button>
            </Tooltip>
...
export default AssetGatePassList;
