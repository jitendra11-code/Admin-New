import { Grid, Stack, TextField, InputAdornment, Tooltip, IconButton, Button } from '@mui/material';
...
function AssetGateInwardList() {
    ...
    const handleExportData = () => {
        axios.get(`${process.env.REACT_APP_BASEURL}/api/AssetGatePass/GetExcelForAssetInward`).then((response) => {
            if (!response) {
    ...
    const getAssetList = async () => {
        await axios
            .get(`${process.env.REACT_APP_BASEURL}/api/AssetGatePass/GetAssetGatePassReturnableDetails`)
            .then((response: any) => {
    ...
    let columnDefs = [
                ...
                <>
                    <div className="actions">
                        <Tooltip title="Edit" className="actionsIcons">
                            <button className="actionsIcons actionsIconsSize" hidden={_validationIdentifierList.includes('request_edit_icon')}>
                                <TbPencil
                                    onClick={() =>
                                        // console.log("eval",e)
                                        navigate(
                                            `/${e?.data?.id}/returnable`,
                        ...
                        <Tooltip title="View" className="actionsIcons actionsIconsSize">
                            <button className="actionsIcons actionsIconsSiz">
                                <VisibilityIcon style={{ fontSize: '15px' }} onClick={() => navigate(`/${e?.data?.id}/returnable`)} />
                            </button>
                        </Tooltip>
                    </div>
                </>
        ...
        },
        {
            ...
            width: 120,
            // minWidth: 120,
            cellStyle: { fontSize: '13px', marginBottom: '15px !important' },
            cellClass: (params) => {
                if (params?.data?.retun_date !== null) {
                    const timeDifference = currentDate.getDate() - new Date(params?.data?.retun_date).getDate();
                    // const daysDifference = Math.floor(timeDifference / (1000 * 3600 * 24));
                    console.log('ZZZ', timeDifference, currentDate.getDate(), new Date(params?.data?.retun_date).getDate());
                    return timeDifference > 0 ? 'highlighted-date' : timeDifference == 0 ? 'current-highlighted-date' : '';
                }
            },
            valueFormatter: (params) => {
                if (params?.data?.retun_date !== null) {
        ...
        },
        {
            ...
            valueFormatter: (params) => {
                if (params?.data?.retun_date !== null) {
                    const timeDifference = currentDate.getDate() - new Date(params?.data?.retun_date).getDate();
                    // const daysDifference = Math.floor(timeDifference / (1000 * 3600 * 24));
                    console.log('ZZZ', timeDifference);
    ...
    return (
        <>
            ...
            <CommonGrid defaultColDef={{ flex: 1 }} columnDefs={columnDefs} rowData={assetData} onGridReady={onGridReady} gridRef={gridRef} pagination={true} paginationPageSize={10} />
        </>
    );
}
