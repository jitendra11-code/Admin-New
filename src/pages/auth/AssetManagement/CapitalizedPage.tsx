...
const CapitalizedPage = ({ }) => {
  ...
  useEffect(() => {
    ...
    {
      setAssetValidation(params?.validation);
      // navigate('/CapitalizedPage/${id}')
    }
}, []);
  ...
  const getAllData = async () => {
    await axios
      .get(`${process.env.REACT_APP_BASEURL}/api/AssetTagging/GetAssetTaggingDetails?id=${id}`)
      .then((response: any) => {
  ...
  useEffect(()=>{
    ...
    setFieldValue("tagNotReceived",AssetData?.tagnotReceived || false);
    setToggleSwitch({
      ...toggleSwitch,
  ...
  const submitCourierDetails = async () => {
    if(values?.Item && values?.AWBNO && values?.courierAgency && values?.DispatchDate && values?.physicallyAvailable && values?.workingCondition && values?.Name && values?.EmailID && values?.Remarks && values?.ConfirmationDate && (values?.tagNotReceived == true ||values?.tagNotReceived==false))
    {
      const body = {
        id: AssetData?.id,
  ...
  return (
    <>
      {' '}
      <Box component="h2" className="page-title-heading mb-0" style={{marginBottom: "5px !important",}}>
            {assetValidation ==="validation" ? "Asset Validation" : "Asset Tagging"}
      </Box>
      <div
        style={{
      ...
      >
        ...
        <div style={{ border: '1px solid rgba(0, 0, 0, 0.12)', marginTop: "10px" }}>
          ...
          <Typography className="section-headpad">Tagging Details</Typography>
                ...
                <>
                  ...
                  <Tooltip title="Download" >
                    <DownloadIcon
                      ...
                      onClick={() => {DownloadFile('AssetImageWithoutTag')}}
              ...
              {imagePreview?.AssetImageWithTag && imageError?.AssetImageWithTag && (
                <>
                  ...
                  <Tooltip title="Download" >
                    <DownloadIcon
                      style={{ fontSize: "20px",marginRight:"25px" }}
                      onClick={() => {DownloadFile('AssetImageWithTag')}}
                      className="closeIocns"
                    />
                  </Tooltip>
              ...
              {imagePreview?.CompleteAssetImage && imageError?.CompleteAssetImage && (
                <>
                  ...
                  <Tooltip title="Download" >
                    <DownloadIcon
                      style={{ fontSize: "20px",marginRight:"25px" }}
                      onClick={() => {DownloadFile('CompleteAssetImage')}}
                      className="closeIocns"
                    />
                  </Tooltip>
      ...
      <div className="bottom-fix-btn" style={{ display: "flex", justifyContent: "center" }}>
        <Grid item xs={6} md={3} sx={{ position: 'relative',width:'500px' }}>
        <TextField
          id="remark"
          name="remark"
          variant="outlined"
