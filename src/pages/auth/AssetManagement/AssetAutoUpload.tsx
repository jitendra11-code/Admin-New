...
import { AutoUploadButton, secondaryButton } from 'shared/constants/CustomColor';
...
const AssetAutoUpload = () => {
    ...
    const documentTypeName = 'Asset Pool Background';
    ...
    useEffect(() => {
        if (documentTypeList && documentTypeList?.length > 0) {
            var obj = documentTypeList && documentTypeList.find((item) => item?.documentName === 'Asset Pool');
            setDocType(obj || null);
        }
    }, [documentTypeList]);
    ...
    const getVersionHistoryData = () => {
        axios
            .get(`${process.env.REACT_APP_BASEURL}/api/ImageStorage/GetDocUploadHistoryForAssetPool?assetcode=0&documentType=${documentTypeName}&entityname=${documentTypeName}&Status=${value === 1 ? 'Completed' : value === 2 ? 'Error' : 'Inprogress'}`)
            .then((response: any) => {
                if (!response) {
                    dispatch(fetchError('Error Occurred !'));
                    return;
            ...
            axios
                ...
                .then((response: any) => {
                    ...
                    if (response?.data?.data == null) {
                        ...
                        getVersionHistoryData();
                        axios
                            .post(`${process.env.REACT_APP_BASEURL}/api/Asset/UploadAndSaveExcelData?recordId=${recId}`, formDataTemp)
                            .then((res) => {
                                if (!res) {
                                    e.target.value = null;
    ...
    const downloadFileLogFile = (data) => {
        ...
        axios
            .get(`${process.env.REACT_APP_BASEURL}/api/Asset/DownloadlogData?RecordId=${data?.recordId || 0}&Type=${value === 1 ? 'Completed' : 'Error'}`)
            .then((response: any) => {
                if (!response) {
                    dispatch(fetchError('Error Occurred !'));
                    return;
                ...
                if (response?.data) {
                    var filename = `Asset_Bulk_Upload_Log(${moment().format('ddd DD/MM/YYYY hh:mm:ss A')}).xlsx`;
                    if (!filename) {
                        dispatch(fetchError('Error Occurred !'));
                        return;
                    }
    ...
    let columnDefs2 = [
        ...
        },
        {
            ...
            width: value == 0 ? 150 : 150,
            minWidth: value == 0 ? 150 : 150,
            cellStyle: { fontSize: '13px' },
        },
        {
            field: 'createdDate',
                        ...
                        <Tooltip title="Download" className="actionsIcons">
                            <DownloadIcon
                                ...
                                onClick={() => {
                                    ...
                                    downloadAssetFile(obj?.data, { id: 0 }, setLoader, dispatch);
                                }}
            ...
            <Grid item xs={7} md={5} sx={{ position: "relative" }}>
              <div>
                ...
                />
                    ...
                    <Box sx={{ borderBottom: 1, borderColor: 'divider', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} style={{ marginTop: '0px' }}>
...
export default AssetAutoUpload;
