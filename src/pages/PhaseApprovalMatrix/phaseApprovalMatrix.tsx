...
export default function PhaseApprovalMatrix() {
  ...
  const [radioButtonMatrix, setRadioButtonMatrix] = useState<any>("Budgeted");
  const [radioButtonStatus, setRadioButtonStatus] = useState<any>("");
  const [radioButtonAction, setRadioButtonAction] = useState<any>({id :0 , userStatus:"" });
  ...
  const handleRadioChangeMatrix = (event) => {
    setRadioButtonMatrix(event.target.value);
  };

  // const handleRadioChangeAction = (e) => {
  //   // setRadioButtonAction(event.target.value);
  //   console.log('setRadioButtonAction', radioButtonAction,e,e.target.value)
  //   setRadioButtonAction({id: params?.data?.id , userStatus : e?.target?.value});
  //   setRadioButtonStatus(e?.target?.value)
  // };
  console.log('setRadioButtonAction', radioButtonAction)
  // const handleRadioChangeAction = (event: any, newAlignment: string) => {
  //   console.log('event',event, newAlignment)
  //   setToggleSwitch({
  //     ...toggleSwitch,
  ...
  const handleRadioValueChanged = (rowIndex, userStatus) => {
    // Update the row data with the new radio button value
    const updatedData = RighttSideData.map((row, index) =>
      index === rowIndex ? { ...row, userStatus} : { ...row, userStatus: "Active" }
    );
    setRighttSideData(updatedData);
  };
  ...
  const getLeftSideData = async () => {
    await axios
      .get(`${process.env.REACT_APP_BASEURL}/api/PhaseApprovalNote/GetPhaseApprovalNoteMatrix?queryType=${radioButton} Phase Note`)
      .then((response: any) => {
  ...
  const showRightData = async  (data:any) => {
    ...
    await axios
    .get(`${process.env.REACT_APP_BASEURL}/api/PhaseApprovalNote/GetUsersAccordingMatrix?queryType=${radioButton} Phase Note&isBudgeted=${radioButton == "Budgeted" ? true : false}&MinValue=${data?.minValue}&MaxValue=${data?.maxValue}&sequence=${data?.sequence}&RoleName=${data?.roleName}`)
    .then((response: any) => {
  ...
  const onAddUser = async() => {
    ...
    await axios
    .post(`${process.env.REACT_APP_BASEURL}/api/PhaseApprovalNote/InsertUsersListInPhaseMatrix`, body)
    .then((response: any) => {
      setOpen(false)
...
console.log('selectedrolematrix',selectedRoleMatrix,selectedUserMatrix)


 

    const roleNameMatrix = selectedRoleMatrix?.role_Name || "";
    const matrixId = selectedRoleMatrix?.id || null;
      ...
      if (parseInt(minValueMatrix) >= parseInt(maxValueMatrix)) {
        ...
        arr?.map((item) => {
            // if (item?.roleName === selectedRoleMatrix?.role_Name && ((minValueMatrix >= item?.minValue && minValueMatrix <= item?.maxValue) || (maxValueMatrix >= item?.minValue && maxValueMatrix <= item?.maxValue))) {
              ...
              console.log('arryRole',arryRole,arryRole?.includes(roleNameMatrix))
                if (((minValueMatrix >= item?.minValue && minValueMatrix <= item?.maxValue) || (maxValueMatrix >= item?.minValue && maxValueMatrix <= item?.maxValue)) 
                 && arryRole?.includes(roleNameMatrix) === true)
                {
                  flag = true;
        ...
        return {
          ...
          roleName: roleNameMatrix || "" ,
          apiType: "Add New Matrix",
          userId: item?.id || 0,
          isBudgeted: radioButtonMatrix == "Budgeted" ? true : false,
          status: matrixStatus || "",
          userStatus:"Active",
          createdBy: user?.UserName,
          createdDate: moment().format("YYYY-MM-DDTHH:mm:ss.SSS"),
          ...
          <div className="toggle-div">
           <ToggleButtonGroup
            ...
            onChange={(e,value)=> { RighttSideData[params?.rowIndex].userStatus = value;
              axios
              .post(`${process.env.REACT_APP_BASEURL}/api/PhaseApprovalNote/UpdateUserStatusInPhaseMatrix?id=${params?.data?.id}&status=${RighttSideData[params?.rowIndex].userStatus}`)
              .then((response : any)=> {
               ...
               console.log('e',e,radioButtonStatus,params, RighttSideData[params?.rowIndex].userStatus)}}
            aria-label="Platform"
          >
  ...
  const handleClickOpen =async () => {
    ...
    await axios
    .get(`${process.env.REACT_APP_BASEURL}/api/PhaseApprovalNote/AddUsersInPhaseMatrix?queryType=${radioButton} Phase Note&isBudgeted=${radioButton == "Budgeted" ? true : false}&MinValue=${selectedLeftsideData?.minValue}&MaxValue=${selectedLeftsideData?.maxValue}&sequence=${selectedLeftsideData?.sequence}&RoleName=${selectedLeftsideData?.roleName}`)
    .then((response: any) => {
  ...
  // const handleKeyPress = (event) => {
  //  if (
  //       (event.key === '-' || event.key === '+' || event.key === 'e' || (event.key === 'ArrowDown' && event.target.value >= 0)) ||
  //       (event.key === 'ArrowUp')
  //   ) {
  //       event.preventDefault();
  //   }
  ...
  useEffect(() => {
    if (!insertRef.current) {
      ...
      axios
        .get (`${process.env.REACT_APP_BASEURL}/api/User/UsersByRoleId?Id=${matrixId}`)
        .then((response : any )=> {
  ...
  const handleExportData = () => {
    axios
    .get(
      `${process.env.REACT_APP_BASEURL}/api/PhaseApprovalNote/ExcelDownloadForApprovalMatrix?queryType=${radioButton} Phase Note`
    )
      .then((response) => {
        if (!response) {
        ...
        if (response?.data) {
          var filename = radioButton+"PhaseApprovalMatrix.xlsx";
          ...
          if (typeof window.navigator.msSaveBlob !== "undefined") {
            ...
            dispatch(
              showMessage(radioButton+ " Phase Approval Matrix data downloaded successfully!")
            );
          }
        }
      });
  };
console.log('radioButtonMatrix',radioButtonMatrix,minValueMatrix,maxValueMatrix,sequenceMatrix)
  return (
    <>
      <Grid
        sx={{
        ...
        <div className="rolem-grid">
          <Stack
            ...
            >
              ...
              sx={{ color: "#fff", fontSize: "12px" , marginRight : "50px"}}
      ...
      >
        ...
        <FormControlLabel value={"Budgeted" } control={<Radio />} label="Budget" />
        <FormControlLabel value={ "NonBudgeted"} control={<Radio />} label="Non Budget" />
      </RadioGroup>
            <Button
              size="small"
              style={primaryButtonSm}
            ...
            >
              ...
              <div className="input-form">
                  ...
                  <TextField
                    ...
                    // onChange={(e:any)=> {setMinValueMatrix(e?.target?.value)}}
                    // disabled
                    // onBlur={handleBlur}
                  />
              ...
              <div className="input-form">
                  ...
                  //   onChange={(e: any) => {
                  //     console.log('PPP', e?.target?.value, e?.target?.value >= 0);
                  //     // We use parseFloat to handle decimal numbers as well
                  //     const newValue = parseFloat(e.target.value);
                  //     // Check if the new value is a number or empty
                  //     if (!isNaN(newValue) || e.target.value === "") {
                  //       setMaxValueMatrix(newValue);
                  //     }
                  // }}
                    // disabled
                    // onBlur={handleBlur}
              ...
              <div>
                <RadioGroup
                  aria-labelledby="demo-controlled-radio-buttons-group"
                  name="controlled-radio-buttons-group"
                  value={radioButtonMatrix}
                  onChange={handleRadioChangeMatrix}
                  style={{display: 'flex', flexDirection: 'row'}}
                >
