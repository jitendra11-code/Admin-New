...
export default function TierMaster() {
    ...
    const handleStatusChange = async (buttonName) => {
        if (gridRef2.current && gridRef2.current.api) {
            ...
            axios
                ...
                .then((response: any) => {
                    if (!response) {
                        dispatch(fetchError('Tier Master Status not updated'));
                        return;
                    }
                    if (response && response?.data?.code === 200 && response?.data?.status === true) {
                        ...
                        dispatch(showMessage('Tier Master Status updated successfully'));
                        gridApi.deselectAll();
                        return;
                    } else {
                        dispatch(fetchError('Tier Master Status already exists'));
                        return;
                    }
                })
                .catch((e: any) => {
    ...
    useEffect(() => {
        if (editFlag) {
            ...
            setFieldValue('tierName', EditMaster?.tierName);
        }
    ...
    const { values, handleBlur, setFieldValue, handleChange, handleSubmit, errors, touched, resetForm } = useFormik({
        ...
        onSubmit: (values, action) => {
            ...
            if (editFlag) {
                axios
                    .post(`${process.env.REACT_APP_BASEURL}/api/TierMaster/InsertUpdateTierMaster`, body)
                    .then((response: any) => {
                        if (!response) {
                            dispatch(fetchError('Tier Master details not updated'));
                            setOpen(false);
                            return;
                        }
                        if (response && response?.data?.code === 200 && response?.data?.status === true) {
                            dispatch(showMessage('Tier Master details updated successfully'));
                            setOpen(false);
                            showRightData();
                            return;
                        } else {
                            dispatch(fetchError('Tier Master details already exists'));
                            setOpen(false);
                            return;
                        }
                    })
                ...
                axios
                    ...
                    .then((response: any) => {
                        if (!response) {
                            dispatch(fetchError('Tier Master details not added'));
                            setOpen(false);
                            return;
                        }
                        if (response && response?.data?.code === 200 && response?.data?.status === true) {
                            dispatch(showMessage('Tier Master details added successfully'));
                            setOpen(false);
                            showRightData();
                            getMyGenerateId();
                            return;
    ...
    const showRightData = async () => {
        ...
        await axios
            .get(`${process.env.REACT_APP_BASEURL}/api/TierMaster/GetTierMasterList?id=`)
            .then((response: any) => {
    ...
    const getTitle = () => {
        if (editFlag) {
            return 'Edit Tier Master Details';
        } else {
            return 'Add Tier Master Details';
        }
    };

    const handleExportData = () => {
        axios.get(`${process.env.REACT_APP_BASEURL}/api/TierMaster/GetExcelReport`).then((response) => {
            if (!response) {
                dispatch(fetchError('Error occurred in Export !!!'));
                return;
            }
            if (response?.data) {
                var filename = 'TierMaster.xlsx';
                ...
                if (typeof window.navigator.msSaveBlob !== 'undefined') {
                    ...
                    dispatch(showMessage('tier Master Excel downloaded successfully!'));
                }
            }
        });
    };
                  ...
                  Add New
                    ...
                    /> */}
                                        ...
                                        <TextField
                                            ...
                                            onChange={handleChange}
                                        />
                                        {touched.tierName && errors.tierName ? <p className="form-error">{errors.tierName}</p> : null}
                                    </DialogContent>
