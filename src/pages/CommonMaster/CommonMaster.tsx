...
export default function Master() {
    ...
    const {
      ...
      onSubmit: (values, action) => {
        ...
        {
          axios
          .post(`${process.env.REACT_APP_BASEURL}/api/FormMaster/UpdateFormMaster`, body)
          .then((response: any) => {
            if (!response) {
              dispatch(fetchError("Common Master details not updated"));
              setOpen(false);
              return;
            };
            if (response && response?.data?.code === 200 && response?.data?.status === true) {
              dispatch(showMessage("Common Master details updated successfully"));
              setOpen(false);
              showRightData(body);
              return;
            } else {
              ...
              >
                <form onSubmit={handleSubmit}>
                  ...
                  <DialogContent style={{ width: "450px" }}>
                    ...
                    <h2 className="phaseLable required" style={styling}>Name</h2>
                    <TextField
                      id="formName"
                      name="formName"
                      variant="outlined"
                      ...
                      onChange={handleChange}
                    />
                    {touched.formName && errors.formName ? (<p className="form-error">{errors.formName}</p>) : null}
                  </DialogContent>
