...
export default function BusinessTypeMaster() {
  ...
  const handleStatusChange = async (buttonName) => {
    if (gridRef2.current && gridRef2.current.api) {
      ...
      axios
        .post(
          `${process.env.REACT_APP_BASEURL}/api/TierMaster/UpdateTierMasterList`,
          selectedData
        )
        .then((response: any) => {
          if (!response) {
            dispatch(fetchError("Admin Vertical List Status not updated"));
            return;
          }
          if (
            response &&
            ...
            dispatch(showMessage("Admin Vertical List Status updated successfully"));
            gridApi.deselectAll();
            return;
          } else {
            dispatch(fetchError("Admin Vertical List Name already exists"));
            return;
          }
        })
        .catch((e: any) => {
  ...
  useEffect(() => {
    if (editFlag) {
      ...
      setFieldValue("admin_Vertical_List_Name", EditMaster?.admin_Vertical_List_Name);
    }
  ...
  let columnDefs2 = [
    ...
    },
    {
      ...
      headerName: "Admin Vertical List Name",
      headerTooltip: "Admin Vertical List Name",
      sortable: true,
      resizable: true,    
      width: 'flex', 
      cellStyle: { fontSize: "13px" },
  ...
  const validateSchema = Yup.object({
    admin_Vertical_List_Name: Yup.string().required("Please enter Admin Vertical List Name"),
  });
  ...
  const {
    ...
    onSubmit: (values, action) => {
      const body = {
        ...
        admin_Vertical_List_Name: values?.admin_Vertical_List_Name,
      ...
      if (editFlag) {
        axios
          .post(
            `${process.env.REACT_APP_BASEURL}/api/AdminVerticalListMaster/InsertUpdateAdminVerticalListMaster`,
            body
          )
          .then((response: any) => {
            if (!response) {
              dispatch(fetchError("Admin Vertical List Name details not updated"));
              setOpen(false);
              return;
            }
            if (
              ...
              dispatch(showMessage("Admin Vertical List Name details updated successfully"));
              setOpen(false);
              showRightData();
              return;
            } else {
              dispatch(fetchError("Admin Vertical List Name details already exists"));
              setOpen(false);
              return;
            }
          })
        ...
        axios
          ...
          .then((response: any) => {
            if (!response) {
              dispatch(fetchError("Admin Vertical List details not added"));
              setOpen(false);
              return;
            }
            if (
              ...
              dispatch(showMessage("Admin Vertical List details added successfully"));
              setOpen(false);
              showRightData();
              // getMyGenerateId();
              return;
            } else {
              dispatch(fetchError("Admin Vertical List details already exists"));
              setOpen(false);
              return;
            }
          })
  ...
  const showRightData = async () => {
    ...
    await axios
    .get(
      // '${process.env.REACT_APP_BASEURL}/api/AdminVerticalListMaster/GetAdminVerticalList'
      `${process.env.REACT_APP_BASEURL}/api/AdminVerticalListMaster/GetAdminVerticalList`
  ...
  const getTitle = () => {
    if (editFlag) {
      return "Edit Admin Vertical List Details";
    } else {
      return "Add Admin Vertical List Details";
    }
  };
  ...
  const handleExportData = () => {
    axios
      .get(
        `${process.env.REACT_APP_BASEURL}/api/AdminVerticalListMaster/GetExcelReport`
      )
      .then((response) => {
        if (!response) {
        ...
        if (response?.data) {
          var filename = "AdminVerticalListMaster.xlsx";
          ...
          if (typeof window.navigator.msSaveBlob !== "undefined") {
            ...
            dispatch(
              showMessage("Admin Vertical List Excel downloaded successfully!")
            );
          }
        }
      });
              ...
              >
                <form onSubmit={handleSubmit}>
                  ...
                  <DialogContent style={{ width: "450px" }}>
                    ...
                    Admin Vertical List Name
                    </h2>
                    <TextField
                      id="admin_Vertical_List_Name"
                      name="admin_Vertical_List_Name"
                      variant="outlined"
                      size="small"
                      value={values?.admin_Vertical_List_Name || ""}
                      onBlur={handleBlur}
                      // onKeyDown={(e: any) => {
                      //   regExpressionTextField(e);
                      // }}
                      ...
                      onChange={handleChange}
                    />
                    {touched.admin_Vertical_List_Name && errors.admin_Vertical_List_Name ? (
                      <p className="form-error">{errors.admin_Vertical_List_Name}</p>
                    ) : null}
                  </DialogContent>
